...
```{r}
preview_filtered_sobj(sobj, min_genes = 400, max_genes = 2000, max_pct_mito = 20)
...
```{r}
sobj <- commit_filtered_sobj(sobj, min_genes = 400, max_genes = 3000, max_pct_mito = 20)



sobj

subsample_cells <- function(seurat_obj, n_cells) {
  
  # Ensure that n_cells is not greater than the total number of cells in the Seurat object
  n_cells <- min(n_cells, ncol(seurat_obj))
  
  # Get the cell names
  cell_names <- colnames(seurat_obj)
  
  # Randomly select n_cells cell names
  selected_cells <- sample(cell_names, n_cells)
  
  # Subset the Seurat object to keep only the selected cells
  subsetted_seurat_obj <- subset(seurat_obj, cells = selected_cells)
